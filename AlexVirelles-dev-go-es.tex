...
\resumeSubheading
    ...
    \resumeItemListStart
        \item Creando \href{https://vesting.cooltech.quest}{cooltech arranger}, una plataforma decentralizada de vesting para tokens erc20 
          con distribucion automatica que se integra a la blockchain(binance) y maneja dinamicamente
          la creacion de recursos de aws(api gateway, lambda, event bridge, dynamodb) usando aws go sdk

        \item Desarrolando un \href{https://dp2p.cooltech.quest}{p2p} decentralizado usando sveltekit para la UI y usando
          vercel para los deployment(tambien experiencia con aws cloudfront)

        \item Desarrollo de REST APIs y servicios backend con go usando el framework echo
          y protocol buffers para la comunicacion entre servicios via grpc(tambien experiencia
          con typescript bun,node,hono)
    \resumeItemListEnd 
...
\resumeSubheading
    ...
    \resumeItemListStart
        ...
        \item Construyendo una \href{https://github.com/Fusyona/web3-core}{libreria} de typescript para hacer fluida la intereaccion entre frontend
          y backend usando SDKs

        \item Configurando un flujo de trabajo CI/CD con github actions que detecta vulnerabilidades al crear PRs
...
\resumeItemListStart
    \vspace{0.8mm}
    \item Construyendo un \href{https://github.com/TheSnakeWitcher/AuctionHub-server}{servidor simple} en golang el cual usa el framework web echo
      para realizar operaciones CRUD usando REST API para interactuar con un servicio de subastas 
